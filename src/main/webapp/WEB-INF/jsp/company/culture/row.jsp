...
<%-- 	<td>${vo.content }</td> --%>
	...
	<td>
		<c:if test="${vo.status != '1' }">
			<a class="edit-a" href="${rootUrl}company/culture/update/${vo.id}">修改</a>
		</c:if>
		<c:if test="${vo.status == '1' }">
			<a class="edit-a" href="${rootUrl}company/culture/update/${vo.id}">详情</a>
		</c:if>
		<a class="add-a"  data-href="${rootUrl}company/culture/delete/${vo.id}" onClick="javascript:aiidc_del(this)">删除</a>
	</td>
</tr>
